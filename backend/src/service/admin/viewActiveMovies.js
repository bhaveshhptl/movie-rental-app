...
export const viewActiveMovies = async () => {
  try {
    ...
    const now = new Date();
    const users = new Map(usersResponse.data.map((user) => [String(user.id), user.name]));
    ...
    const err = new Error("Unable to fetch active movies");
